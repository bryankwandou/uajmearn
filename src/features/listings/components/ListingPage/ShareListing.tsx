...
function MainContent({ listing, grant, source }: SourceType) {
  ...
  const shareMessage = (
    ...
    let copy = '';
    if (source === 'grant') {
      copy = `Just came across this massive grant opportunity by ${grant?.sponsor?.name} on UAJM Earn`;
    } else {
      if (listing?.type !== 'project') {
        copy = `Just came across this banger${!!listing?.usdValue ? ` $${listing?.usdValue}` : ''} bounty by ${listing?.sponsor?.name} on UAJM Earn`;
      } else {
        copy = `Just came across this gig by ${listing?.sponsor?.name} on UAJM Earn`;
      }
    }
    if (
