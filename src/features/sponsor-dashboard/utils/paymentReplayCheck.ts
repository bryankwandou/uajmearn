...
function buildJsonSearchExistsQuery(
  ...
  return (txId: string) => {
    const searchPredicates = paymentTxSearchPatterns(txId).map(
      (pattern) =>
        Prisma.sql`EXISTS (SELECT 1 FROM jsonb_array_elements(CASE WHEN jsonb_typeof(paymentdetails::jsonb) = 'array' THEN paymentdetails::jsonb ELSE '[]'::jsonb END) AS pd(elem) WHERE pd.elem->>'txId' LIKE ${pattern})`,
    );

    return Prisma.sql`
      EXISTS (
        SELECT 1
        FROM ${Prisma.raw(`"${tableName.toLowerCase()}"`)}
