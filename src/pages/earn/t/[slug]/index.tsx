...
function TalentProfile({ talent, stats, bgIndex, shouldNoIndex }: TalentProps) {
  ...
  const description = useMemo(() => {
    ...
    ).toLocaleString()} earned, ${stats?.wins || 0} wins on UAJM Earn.`;

    return parts.length > 0
      ? `${parts.join(' | ')}. ${statsStr}`
      : `Talent profile on UAJM Earn. ${statsStr}`;
  ...
  const title =
    talent?.firstName && talent?.lastName
      ? `${talent?.firstName} ${talent?.lastName} | UAJM Earn Talent`
      : 'UAJM Earn';
    ...
    const [talent, submissionStats, grantStats] = await Promise.all([
      ...
      prisma.$queryRaw<SubmissionStats[]>`
        SELECT
          ...
          SUM(CASE WHEN s.isWinner = true AND b.isWinnersAnnounced = true THEN 1 ELSE 0 END) as wins,
          COALESCE(SUM(CASE WHEN s.isWinner = true AND b.isWinnersAnnounced = true THEN s.rewardInUSD ELSE 0 END), 0) as "listingWinnings"
        FROM Submission s
